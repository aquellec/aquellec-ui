...
import { accentTileClass, anchorSlug, cardSurface, softTransition } from './_showcase';
...
function Swatch({ scaleName, shade, hex }: { scaleName: string; shade: string; hex: string }) {
  ...
  return (
    <div className="flex flex-col">
      {/*
        One identical hairline on every swatch. The key shades used to carry an
        extra `ring-2`, which stayed almost invisible on white but turned into a
        thick grey double frame on a dark surface — and it was redundant: the
      ...
      */}
      ...
      {role && <p className="text-xs font-medium text-brand-700 dark:text-brand-300">{role}</p>}
    </div>
  );
}
    ...
    <section className={cn('sb-unstyled mb-4 p-5', cardSurface)}>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {Object.entries(aquellecThemeExtensions.borderRadius).map(([token, value]) => (
          <div key={token}>
            <div
              className="mb-1.5 h-16 w-full border border-brand-500/30 bg-brand-50 dark:border-brand-400/30 dark:bg-brand-500/15"
...
export function TypeScale() {
  ...
  return (
    <section className={cn('sb-unstyled mb-4 p-5', cardSurface)}>
      <div className="mb-4 flex items-center gap-2.5">
        <span className={cn('inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg', accentTileClass.info)}>
